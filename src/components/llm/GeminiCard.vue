  ...
  <Card class="provider-card">
    <template #content>
      <div class="row row-between">
        <div class="row">
          <Avatar icon="pi pi-star" shape="square" size="large" class="gemini-avatar" />
          <div>
            <h3 class="title">Gemini</h3>
            <small class="subtitle">Connect your Google Gemini API for multimodal AI.</small>
          </div>
        </div>
...
function connectProvider() {
  ...
  emit('connect', { provider: 'gemini', api_key: apiKey.value })
  apiKey.value = ''
}

function disconnectProvider() {
  emit('disconnect', 'gemini')
}
</script>
